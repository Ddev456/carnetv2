...
import { plantActionEditContent } from "../plant.action";
...
export const MdxEditor = ({ plantId, markdown }: MdxEditorProps) => {
  const [syncState, setSyncState] = useState<SyncState>("sync");

  const onChange = useDebounceFn(async (value: string) => {
    setSyncState("syncing");

    const { data, serverError } = await plantActionEditContent({
      plantId,
      markdown: value,
    });
